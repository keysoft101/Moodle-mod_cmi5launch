...
A plugin for Moodle that enables tracking for cmi5 content. This plugin acts as a bridge between Moodle and a cmi5 player, as well as Moodle and an LRS, enabling recording and tracking of cmi5 statements. It utilizes the CATAPULT suite's cmi5 player which launches the content as well as handles AU satisfaction statements. It also links with an LRS to display those statements and others generated by moodle.

Users can upload the cmi5 content as a Moodle activity. It also integrates with Moodle's Gradebook API for grading purposes.
...
- LRS integration
  - This is an optional dropdown for additional LRS's. Defaults to 'None.'
...
- Maximum grade
  - The Maximum the grade can be (default 100).
...
| name        | char          | Name field to hold the course name in Moodle. This may be different than the course name in its own xml file. |
...
| launchmethod | char         | Whether the course's retrieved URL opens in its own window or a new one |
...
| launchmode  | char          | Launch mode, which is separate from launchmethod. It can be 'Normal', 'Browse', or 'Review' |
...
| launchmethod | char         | Whether the course's retrieved URL opens in its own window or not |
